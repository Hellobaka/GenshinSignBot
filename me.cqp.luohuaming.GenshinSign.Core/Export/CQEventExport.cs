...
using me.cqp.luohuaming.GenshinSign.Core;
using me.cqp.luohuaming.GenshinSign.Core.Domain;
using me.cqp.luohuaming.GenshinSign.Sdk.Cqp;
using me.cqp.luohuaming.GenshinSign.Sdk.Cqp.Enum;
using me.cqp.luohuaming.GenshinSign.Sdk.Cqp.EventArgs;
using me.cqp.luohuaming.GenshinSign.Sdk.Cqp.Interface;
using me.cqp.luohuaming.GenshinSign.Sdk.Cqp.Expand;
using me.cqp.luohuaming.GenshinSign.Sdk.Cqp.Model;
using Unity;
using Unity.Injection;

namespace me.cqp.luohuaming.GenshinSign.App.Export
{
	/// <summary>	
	/// 表示酷Q事件导出的类	
	/// </summary>	
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.GenshinSign";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.GenshinSign", 1, 9, "原神签到机", "1.0.0", 1, "落花茗", "每日自动进行米游社原神签到", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.GenshinSign", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.GenshinSign", AppData.CQLog);	
